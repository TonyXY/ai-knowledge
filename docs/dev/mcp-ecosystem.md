...
| 仓库 | ⭐ Stars | 技能数 | 推荐理由 | 安装方式 |
|------|---------|--------|---------|---------|
| **obra/superpowers** | 161K | 14+ 个 | 完整的软件开发方法论，含 TDD、系统调试、代码审查、并行 Agent 开发等工作流 | `curl -fsSL https://raw.githubusercontent.com/obra/superpowers/refs/heads/main/scripts/install.sh \| bash` |
| **affaan-m/everything-claude-code** | 182K | 183 个 | 最全的技能库——48 个 Agent、183 个技能、79 个命令、跨 Claude Code/Cursor/OpenCode/Codex | `git clone && ./install.sh` |
| **addyosmani/agent-skills** | 44K | 23 个 | Google 工程文化沉淀——生产级工程技能，含架构设计、安全审查、性能优化、API 规范 | `gemini skills install ./agent-skills/skills/` |
| **davila7/claude-code-templates** | 27K | 100+ 个 | 即装即用的 Agent/命令/MCP 合集，支持按需选择 | `npx claude-code-templates@latest --agent <name>` |
| **alirezarezvani/claude-skills** | 15K | 313+ 个 | 跨 12 个 AI 编程工具的技能库，覆盖工程/营销/产品/合规/金融等领域 | `git clone && ./scripts/install.sh --tool <tool>` |
